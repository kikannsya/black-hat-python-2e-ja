...
def handler(chan, host, port):
    #ローカルネットワーク内をソケットによって通信する
    #クライアントがローカルネットワークとリモートSSHサーバーのポートフォワードを仲介
    sock = socket.socket()
    try:
...
def reverse_forward_tunnel(server_port, remote_host, remote_port, transport):
    #SSHサーバー（外部ネットワーク）のポートからTCP接続を転送
...
def main():
    ...
    try:
        # SSH接続はここでしている
        # クライアント(対象ネットワークのローカルホスト)からSSHサーバーにSSHリクエスト
        client.connect(
            server[0],
            server[1],
    ...
    # コネクションを確立した後、revrese_forwrd_tunnelでSSHで暗号化した通信でChannnelで通信
    try:
